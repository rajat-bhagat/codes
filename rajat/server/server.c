...
int q,count=0,max_client=0,i=0;
char str[1024],str1[1024],name[20];
char cname[20];
...
int main()
{
	...
	while(1)
	{
		...
		{	
			...
			bzero(str,1024);
			pthread_create(&tid[count++],NULL,&func_read,&database[i]); // creating the read thread to perform the read operation
			printf("count: %d\n",count);
